...
void GuiFileDialog(GuiFileDialogState *state)
{
    if (state->fileDialogActive)
    {
        ...
        state->fileDialogActive = !GuiWindowBox((Rectangle){state->position.x + 0, state->position.y + 0, winWidth, winHeight}, "Select a save file");
    ...
    if (dirFilesCount > 1)
    {
        ...
        for (;;)
        {
            for (; left + 1 < len; len++) // Sort left to len - 1
            {
                ...
                {
                    while (FileCompare(files[++right], pivot, dir) < 0)
                        ; // Look for greater element
                    while (FileCompare(pivot, files[--len], dir) < 0)
